...
class heartBeatClass(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        self.log('train_acc', self.train_acc(pred,y), on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        #y_hat = [p.item() for p in y]
        self.log('val_loss', loss, on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
        self.log('val_acc', self.valid_acc(pred,y), on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
        #self.log('val_confusion_matrix', self.valid_confusion_matrix(preds, y_hat), on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
    ...
    def predict_step(self, test_batch, batch_idx):
        ...
        #self.log('test_acc', self.test_acc(pred,y), on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
        #self.log('test_confusion_matrix', self.test_confusion_matrix(pred, y), on_epoch=True, sync_dist=True, prog_bar=True, logger=True)
        return pred,y
...
if __name__ == '__main__':
    ...
    args.add_argument('--checkpoint_folder', type=str, default='ckpts/', help='path to save checkpoints')
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(monitor='val_loss', save_top_k=5, mode='min', 
                                                       dirpath=f"ckpts/lr{args.lr}-batch{args.batch_size}/",
                                                       filename='model-{epoch:02d}-{val_loss:.2f}')
    trainer = pl.Trainer(devices='auto', accelerator='gpu', max_epochs=args.max_epochs, logger=wandb_logger,
            log_every_n_steps=1, deterministic=True, callbacks=[pl.callbacks.early_stopping.EarlyStopping
            (monitor='val_loss', patience=10, mode='min'), checkpoint_callback],default_root_dir=args.checkpoint_folder)
